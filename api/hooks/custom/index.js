      ...
      before: {
        '/*': {
          ...
          fn: async function (req, res, next) {
            ...
            // Dont have token? proceed as usual
            ...
            // verify token
            try {
              var decoded = await jwt.verify(token, 'motconvitxoerahaicaicanh');
            } catch(err) {
              sails.log(err);
              return next();
            }
``
            ...
            // try {
            //   var decoded = jwt.verify(token, 'motconvitxoerahaicaicanh');
            //   var loggedInUser = await User.findOne({
            //     id: decoded.userId
            //   });
            // } catch (err) {
            //   sails.log('loi roi ma');
            // }

            // If the logged-in user has gone missing, log a warning,
            // wipe the user id from the requesting user agent's session,
            // and then send the "unauthorized" response.
            // đoạn này giờ ko còn ý nghĩa nữa
            // if (!loggedInUser) {
            //   sails.log.warn('Somehow, the user record for the logged-in user (`' + req.session.userId + '`) has gone missing....');
            //   delete req.session.userId;
            //   return res.unauthorized();
